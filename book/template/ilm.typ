...
#let ilm(
  ...
  show raw: set text(font: "Zenbones Brainy", size: 9pt)

  // Configure page size and margins.
  set page(
    paper: paper-size,
    margin: (x: 2.35cm, top: 2.1cm, bottom: 2.2cm),
  )
  ...
  set par(leading: 0.84em, spacing: 1.05em, justify: false, linebreaks: "optimized")
  ...
  // will only apply to body.
  {
    ...
    show heading.where(level: 1): set text(size: 1.65em, weight: "semibold")
    show heading.where(level: 2): set text(size: 1.28em, weight: "semibold")
    show heading.where(level: 3): set text(size: 1.08em, weight: "semibold")
